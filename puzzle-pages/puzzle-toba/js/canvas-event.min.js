...
(function(window, doc, undefined) {
  ...
  var toString = Object.prototype.toString,
    ...
    getObjectUnderCursor = function(e, self) {
      ...
      var shape,
        ...
        i = shapes.length;
      self.x =
        (((e && e.pageX - self._pos.x + 1) || window.event.offsetX + 1) - 1) /
          self.__zoom || 0;
      self.y =
        (((e && e.pageY - self._pos.y + 1) || window.event.offsetY + 1) - 1) /
          self.__zoom || 0;
      ...
      return function(type, self) {
        ...
        return function(e) {
          ...
          var special = hotkeys.specialKeys[code],
            character =
              special || String.fromCharCode(code || e.charCode).toLowerCase(),
            modif = "",
            handlers;
          if (e.altKey) {
            modif += "alt+";
  ...
  each(
    (
      ...
      "mouseup click dblclick focus blur tap"
    ).split(" "),
    function(name) {
...
(function(Cevent, window) {
  ...
  var math = Math,
    ...
    distanceToLine = function(x1, y1, x2, y2, point) {
      ...
      u =
        ((point.x - x1) * deltaX + (point.y - y1) * deltaY) /
    ...
    Rect = Shape.extend({
      ...
      hitTest: function(point) {
        ...
        return (
          mousePos.x >= 0 &&
          mousePos.x <= this.w * this.scaleX &&
          mousePos.y >= 0 &&
          mousePos.y <= this.h * this.scaleY
        );
      }
    }),
    Text = Rect.extend({
    ...
    Line = Shape.extend({
      ...
      hitTest: function(point) {
        return (
          distanceToLine(this.x, this.y, this.x2, this.y2, point) <=
          this.lineWidth + 2
        );
      }
    }),
